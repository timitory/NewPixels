        ...
        <div class="items-center flex text-sm mt-4 relative border border-solid">
          ...
          <button @click="showFilter = !showFilter" class="py-2 px-2">Filter by
           ...
           <div v-if="showFilter" class="filter shadow-lg absolute left-0 bg-white">
            <p class="mb-2 cursor-pointer text-sm" @click="filterByPolicyType('Health')">Health</p>
            <p class="mb-2 cursor-pointer text-sm" @click="filterByPolicyType('Vehicle')">Vehicle</p>
            <p class="mb-2 cursor-pointer text-sm" @click="filterByPolicyType('Home-content')">Home-content</p>
            <p class="mb-2 cursor-pointer text-sm" @click="filterByPolicyType('Gadget')">Gadget</p>
          </div> 
        </div>
      </div>
          ...
          <tbody>
            <tr v-for="(policy, index) in filteredPolicies" :key="index" class="border border-solid border-gray-300">
              ...
              <td>
                ...
                <span v-else-if="policy.Status.name== ''" class="rounded text-white text-center p-1 "> </span>
                <span v-else-if="policy.Status.name == 'Active'" class="rounded text-white text-center p-1 bg-blue-500"> {{policy.Status.name}}</span>
                <span v-else-if="policy.Status.name == 'Suspended'" class="rounded text-white text-center p-1 bg-orange-500"> {{policy.Status.name}}</span>
                <span v-else-if="policy.Status.name == 'Expired'" class="rounded text-white text-center p-1 bg-red-500"> {{policy.Status.name}}</span>
              </td>
            </tr>
          </tbody>
        </table>
