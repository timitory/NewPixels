      ...
      <div class="mt-8">
        <div class="mt-8 px-6 pt-6 lg:px-20 pb-20 relative shadow-lg bg-white lg:relative" style="box-shadow: 0px 20px 33px #00000029; min-height: 1300px">
          <div class="justify-between lg:absolute lg:top-2 lg:right-4 lg:flex lg:gap-4">
            <div class="relative">
                <input type="text" v-model="searchKeyword" class="block mt-4 rounded bg-blue-100 px-4 lg:pl-10 py-2 w-full outline-none focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" placeholder="Search policies...">
            ...
            <div class="items-center flex text-sm mt-4 relative border border-solid">
              ...
              <button @click="showFilter = !showFilter" class="py-2 px-2">Filter by
               ...
               <div v-if="showFilter" class="filter shadow-lg absolute left-0 bg-white">
                <p class="mb-2 cursor-pointer text-sm" @click="filterData('Health')">Health</p>
                <p class="mb-2 cursor-pointer text-sm" @click="filterData('Vehicle')">Vehicle</p>
                <p class="mb-2 cursor-pointer text-sm" @click="filterData('Home-content')">Home-content</p>
                <p class="mb-2 cursor-pointer text-sm" @click="filterData('Gadget')">Gadget</p>
              </div> 
            </div>
          </div>
          <div class="mt-4 lg:mt-0 flex items-end"></div>
            ...
            <table v-if="filteredPolicies.length > 0" class="w-full mt-8">
              ...
              <tbody>
                <tr v-for="(policy, index) in filteredPolicies" :key="index" class="border border-solid border-gray-300">
                  <td>{{index + 1}}</td>
                  <td>{{policy.enrollee.firstname}}</td>
                  <td>{{policy.enrollee.lastname}}</td>
                  <td>{{policy.HealthPlan.name}}</td>
                  <td>
                    <span v-if="policy.Status.name == 'Success'" class="rounded text-white text-center p-1 bg-green-500">{{policy.Status.name}}</span>
                    <span v-else-if="policy.Status.name == 'Active'" class="rounded text-white text-center p-1 bg-green-500">{{policy.Status.name}}</span>
                    <span v-else-if="policy.Status.name == 'Pending'" class="rounded text-white text-center p-1 bg-yellow-500">{{policy.Status.name}}</span>
                    <span v-else class="rounded text-white text-center p-1 bg-red-500">{{policy.Status.name}}</span>
                  </td>
                </tr>
              </tbody>
            </table>
            <p v-else class="mt-8 text-center">No policies found.</p>
          </div>
          <p v-if="totalPages > 1" class="mt-10 mb-6 text-sm text-center">Showing page {{currentPage}} of {{totalPages}}</p>
          <div v-if="totalPages > 1" class="flex justify-center mt-6 text-sm">
            <div class="space-x-2">
              <button :disabled="currentPage === 1" @click="currentPage = currentPage - 1" class="px-3 py-2 bg-gray-200 rounded disabled:opacity-50">
                Previous
              </button>
              <button :disabled="currentPage === totalPages" @click="currentPage = currentPage + 1" class="px-3 py-2 bg-gray-200 rounded disabled:opacity-50">
                Next
              </button>
            </div>
          </div>
...
export default {
  data() {
    return {
      policies: [], // Assuming policies will be loaded from an API or data source
      searchKeyword: '',
      showFilter: false,
      selectedFilter: '',
      currentPage: 1,
    ...
    filteredPolicies() {
      ...
      if (this.searchKeyword) {
        ...
        filtered = filtered.filter(policy => 
          policy.enrollee.firstname.toLowerCase().includes(keyword) || 
          policy.enrollee.lastname.toLowerCase().includes(keyword) || 
          policy.HealthPlan.name.toLowerCase().includes(keyword)
        );
      }

      if (this.selectedFilter) {
        filtered = filtered.filter(policy => policy.HealthPlan.name === this.selectedFilter);
      }
