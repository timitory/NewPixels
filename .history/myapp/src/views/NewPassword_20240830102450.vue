        ...
        <div class="flex flex-col w-full  shadow-lg rounded-lg bg-white p-8">
             ...
             <form @submit.prevent="showNext" class="w-full">
                ...
                <div class="mt-4 lg:mt-6 rounded w-full relative">
                  ...
                  <input type="email" placeholder="***********" v-model="user.password" class="mt-2 w-full bg-white py-2 px-3 rounded outline-none border border-slate focus:outline-none focus:ring-1 focus:ring-violet focus:border-transparent" required>
                  <p v-if="error.email" class="text-red-500 text-sm mt-2">Please enter a valid Passsword</p>
               ...
               <h1 class="inline-block mt-6 text-center rounded focus:outline-none text-sm py-2 w-full text-dark"><router-link to="/forgot" class="text-violet">Go Back</router-link></h1> 
              </form>
        </div>
     </div>
   </template>
